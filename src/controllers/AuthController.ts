...
export class AuthController {
  static async auth(req: UserRequest, res: Response) {
    ...
    if (!AuthController.checkIfUnencryptedPasswordIsValid(unencryptedPassword, user.password)) return res.status(401).send('Email or password not valid!');
  ...
  static async changePassword(req: UserRequest, res: Response) {
    ...
    if (!AuthController.checkIfUnencryptedPasswordIsValid(oldPassword, user.password)) return res.status(401).send('Old password do not match');
